...
помощью `canAnyAuthItems` и `canAnyAuthItems`, которые принимают в качестве аргументов два параметра: 
...
По умолчанию кеш отключен, т.к. несмотря на относительно большое количество запросов (от 3 до 5 в зависимости от ситуации)
проверка на допуск осуществляется невероятно быстро ввиду простоты этих запросов к БД (select из таблицы по индексам). Однако, если
вы хотите избавиться по каким-либо причинам от постоянных запросов к БД, вы можете включить кеш, который, в свою очередь,
